...
class Gaussian_spin_distribution():
    def __init__(self, **model_pars):
        ...
        mean = np.array([mu_eff, mu_p])
        cov = np.array([[sigma_eff**2, rho*sigma_eff*sigma_p],[rho*sigma_eff*sigma_p,sigma_p**2]])
    ...
    def marginal_pxeff(self):
        xeff, xp = self.GetGrid()[-2:]
        f = self.lambda_peak
        pm = np.zeros(xeff.size)
        for i in range(xeff.size):
            p = np.array([ (1-f)*self.model.pdf(np.array([xeff[i],xpi]).T)/self.normalization[0] + f*self.narrow_peak_pdf(np.array([xeff[i],xpi]) ) / self.normalization[1] for xpi in xp]) 
            pm[i] =  np.sum(p) * (xp[1]-xp[0])
        norm = np.sum(pm)* (xeff[1]-xeff[0])
        return pm / norm 
    ...
    def marginal_pxp(self):
        ...
        for i in range(xp.size):
            p = np.array([ (1-f)*self.model.pdf(np.array([xeffi,xp[i]]).T)/self.normalization[0] + f*self.narrow_peak_pdf(np.array([xeffi,xp[i]]) ) / self.normalization[1] for xeffi in xeff]) 
            pm[i] =  np.sum(p) * (xeff[1]-xeff[0])
        norm = np.sum(pm)* (xp[1]-xp[0])
        return pm / norm 
