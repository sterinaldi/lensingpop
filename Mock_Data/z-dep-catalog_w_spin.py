...
if __name__ == '__main__':
    ...
    interpolator_file = Path("./selection_functions/selfunc_m1qz_source.pkl").resolve()
    ...
    np.savez(catalog_file, m1=m1, m2=m2, xeff=chi_eff, redshift=redshiftValue, snr=snr, 
    ...
    pdet_value = pdet(np.array([m1,m2/m1,redshiftValue_l]).T)
...
################## Save the data after applying selection effect ########################################################
    np.savez(filtered_catalog_file, m1=m1[index], m2=m2[index], redshift=redshiftValue[index], xeff=chi_eff[index], snr=snr[index],
