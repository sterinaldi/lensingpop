...
if __name__ == '__main__':
    ...
    if options.evaluate_interpolators:
        ...
        pdf_median_source = RegularGridInterpolator(points = (m1, m2, z), values = median_source, method = 'linear',bounds_error=False, fill_value=0) # Make a 3D interpolator
        ...
        pdf_median_detector = RegularGridInterpolator(points = (m1z, m2z, z), values = median_detector, method = 'linear',bounds_error=False, fill_value=0)
    ...
    if options.make_selfuncs:
        ...
        # 3D
        selection_function_m1m2z_source = RegularGridInterpolator(points = (m1,m2,z), values = p_m1m2z_source, method = 'linear',bounds_error=False, fill_value=0) # Creates an interpolator for pdet(theta)
        with open(Path(selfunc_folder, 'selfunc_m1m2z_source.pkl'), 'wb') as f:
            dill.dump(selection_function_m1m2z_source, f)
        # 2D
        selection_function_m1m2_source = RegularGridInterpolator(points = (m1,m2), values = p_m1m2_source, method = 'linear',bounds_error=False, fill_value=0)
