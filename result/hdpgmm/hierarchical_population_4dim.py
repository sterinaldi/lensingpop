...
file = Path("~/desktop/lensingpop/Mock_Data/m1m2zxeffxp_posterior_PPD_afterSelection_unlensed3303.npz")  # input file. modify this line according to your path
...
fivedim_posteriors=[np.array([mass1[(mass1<Mmax) & (xeff<xeff_max) & (xeff_min<xeff) & (xp<xp_max)],
                    mass2[(mass1<Mmax) & (xeff<xeff_max) & (xeff_min<xeff) & (xp<xp_max)],
                    xeff[(mass1<Mmax) & (xeff<xeff_max) & (xeff_min<xeff) & (xp<xp_max)],
                    xp[(mass1<Mmax) & (xeff<xeff_max) & (xeff_min<xeff) & (xp<xp_max)]]).T
                    for mass1,mass2,xeff,xp in zip(data["m1_posterior"],data["m2_posterior"],
                    data["xeff_posterior"],data["xp_posterior"])]

true_sample   = np.array([data["m1"]*(1+data["redshift"]),data["m2"]*(1+data["redshift"]),
                         data["xeff"],data["xp"]]).T # Since we have the true masses, let's use them for the plots
